...
| `@easyx/image-toolkit/ui` | 浏览器侧：引擎客户端、React hooks、图片编辑内容区组件 | React + wasm 引擎（惰性加载），UI 自研 |
...
包内只提供编辑内容区，容器与保存动作都由宿主负责。内容区内部已含 `<ImageEngineGate>`，引擎加载进度、错误与重试无需宿主处理；`onResultChange` 会带上 `{ result, pending, noop, unsupported, error }`，`result` 为 `null` 表示当前设置未产生变更或尚不可保存。
...
编辑内容区随宿主容器渲染，`data-theme` 挂在任意祖先上都会命中；若宿主用类名等 CSS 覆盖不到的方式表达主题，用 `<ImageEditor theme="dark" />` 显式指定。
