...
解析全部在宿主侧完成，包内只消费解析结果：能力经顶层 `tools.parseDocument` 注入，是一个**异步方法** —— 可在浏览器本地解析，也可调用自己的服务端解析接口。
...
传入 `tools.parseDocument` 后，对话输入框出现「添加文档」入口，也可直接把 `.docx` / `.pdf` 粘贴或拖入。文档在**添加时**即开始解析，解析完成前不能发送。
