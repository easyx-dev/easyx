...
- **内置回放（默认）**：文档站是纯静态站点、没有服务端，因此在浏览器侧拦截演示端点，回放一段预录的 OpenAI 标准 SSE 流。对话链路走真实实现：包内适配器消费 SSE、`createChatHook` 驱动消息渲染、流式同步到编辑器与预览
- **真实模型**：填入任意 OpenAI Chat Completions 兼容端点（Base URL + API Key + 模型）即可直接对话；配置写在浏览器 `localStorage`，密钥不离开本机，可用「清除连接」移除
...
- 真实接入时把 `endpointUrl` 指向宿主的 OpenAI 兼容端点、`media` 指向宿主的上传与媒体库接口即可，组件侧无需改动
